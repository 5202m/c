...
var room = {
    ...
    sendWhMsg: function(txtObj) {
        ...
        if (room.userInfo.userType == 1) {
            dasData.clientGroup = 'admin'; //return "管理员";
        } else if (room.userInfo.userType == 2) {
            dasData.clientGroup = 'analyst'; //return "分析师";
    ...
    setWhOnlineTip: function(toUserId, isOnline, userInfoTmp) {
        if (isOnline) {
            if (userInfoTmp) {
                ...
                var nk = $('#wh_msg_' + toUserId + ' .wh-title label').text(),
                    ttp = $('#wh_msg_' + toUserId + ' .wh-title .title-tip');
    ...
    articleNote: {
        ...
        saveArticleNote: function() {
            ...
            articleContent = this.filterContent(articleContent);
            if (!articleContent) {
    ...
    getOnlineUserDom: function(row) {
        ...
        var dialogHtml = room.getDialogHtml(row.clientGroup, row.userId, row.nickname, row.userType, row.isMobile, false),
            isMeHtml = "",
            seq = row.sequence;
    ...
    setOnlineUser: function(row) {
        ...
        var lis = $("#userListId li"),
            onlineUserDom = this.getOnlineUserDom(row),
    ...
    publishViewPoint: function() {
        ...
        var param = {
            category: 'trade_strategy_article',
            platform: room.userInfo.groupId,
    ...
    fillWhHistory: function() {
        room.whHistoryUserObj.whUserList = room.getWhHistory();
        var html = '',
            formatHtmlStr = room.formatHtml('whHistory');
        if (!$.isArray(room.whHistoryUserObj.whUserList)) {
