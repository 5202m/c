...
var chat={
    ...
    setEvent:function(){
        ...
        $("#sendBtn").click(function() {
           ...
           // chat.socket.emit('sendMsg', sendObj);//发送数据
            ...
            chatAnalyze.setUTM(false,$.extend({operationType:2}, indexJS.userInfo, indexJS.courseTick.course));//统计发言次数
        });
        //发送图片--选择图片
     ...
     * @param avatar
     */
    setDialog:function(userId,nickname,talkStyle,userType,avatar){  console.log(userId);
    ...
    setContent:function(data,isMeSend,isLoadData){
        ...
        if(data.rule){
           ...
           //判断弹出层只弹出一次
                if(userGagCooikes != null && userGagCooikes != "" && userGagCooikes>0){
                }else{
...
//        chat.socket.emit('sendMsg',sendObj);//发送数据
        ...
        txtObj.html("");//清空内容
        chatAnalyze.setUTM(false,$.extend({operationType:8}, indexJS.userInfo, indexJS.courseTick.course));//统计发言次数
    },
    /**
     * 设置私聊访客
     * @param userType
            ...
            switch (result.type)
            {
                case 'onlineNum':{
                    ...
                    if(data.online){
                        ...
                        //得到聊天相关属性
                        var lis=$("#userListIdNew li"),onlineUserDom=chat.getOnlineUserDom(userInfoTmp),liDom=onlineUserDom.dom;
