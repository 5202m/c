...
var chatTeacher = {
    ...
    trainDetails : function(obj){
        ...
        var roomName=$this.parent().find(".train_name").text();
        chatTeacher.trainDetailsLoad(userNo, roomName);
    },
    /**培训班详情*/
    trainDetailsLoad : function(userNo, roomName){
        $('.train_detail .pop_tit label').text(roomName);
        var classMap = {
            "joe_chung" : "popup_box tbox train_detail zwytrain dn",
            "tracey_jiang" : "popup_box tbox train_detail cctrain_detail dn",
            "default" : "popup_box tbox train_detail dn"
        };
        $("#panel_popupBox_train").attr("class", classMap.hasOwnProperty(userNo) ? classMap[userNo] : classMap["default"]);
        LazyLoad.css(["/pm/theme1/css/train.min.css"]);
    ...
    trainRegisByUserNo : function(userNo){
        var $obj = $(".pop_train .scrollbox .trainlist a[userno='" + userNo + "']:first");
        if($obj.size() == 0){
            $obj = $("#roomList_panel a[userno='" + userNo + "'][rt='train']")
        }
