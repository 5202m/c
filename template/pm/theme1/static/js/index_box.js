...
var box={
    ...
    loginEvent:function(){
        ...
        $("#logout").bind("click", function(){
            ...
            window.location.href="/studio/logout?platform=" + indexJS.fromPlatform+'&cookieId='+chatAnalyze.getUTMCookie()+'&roomName='+$('#roomInfoId').text()
                    ...
                    if(box.toRoomId){
                        common.getJson("/studio/checkGroupAuth",{groupId:box.toRoomId},function(checkGroupRes){
                            if(!checkGroupRes.isOK){
                                alert("您没有访问该房间的权限，将为您跳转到默认房间，如有疑问，请联系老师助理。");
                            }
                            indexJS.toRefreshView();
                        },true,function(err){
                            if("success"!=err) {
