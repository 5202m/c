...
var videosTrain = {
    ...
    changeRoomOrSignup: function(groupId) {
        common.getJson("/checkGroupAuth", { groupId: groupId }, function(authResult) {
            if (!authResult || !authResult.checkState) {
                indexJS.toRefreshView();
            } else {
                var msg = (authResult.checkState.message || "") + "已为你自动跳转到默认房间。";
                var trainCfg = videosTrain.getTrainConfig(authResult && authResult.trainConfig);
                if (authResult.roomType == "train" && !trainCfg && authResult.checkState.code == "4007") {
                    common.getJson('/addClientTrain', {
                        groupId: groupId,
                        noApprove: 1
                    }, function(addTrainResult) {
                        if (!addTrainResult || addTrainResult.code == "4016") {
                            common.getJson("/checkGroupAuth", { groupId: groupId }, function(nextAuthResult) {
                                if (!nextAuthResult || !nextAuthResult.checkState) {
                                    indexJS.toRefreshView();
                                } else {
                                    alert(msg);
                                    indexJS.toRefreshView();
                            ...
                            alert(addTrainResult.message + "已为你自动跳转到默认房间。");
                            indexJS.toRefreshView();
                        }
                    });
                } else {
    ...
    getTrainList: function() {
        $.getJSON('/getTrainRoomList', { groupType: indexJS.userInfo.groupType }, function(result) {
            if (result != null) {
                ...
                var trainHtml = [],
                    trainEndHtml = [],
                    trainFormatHtml = videosTrain.formatHtml('train');
