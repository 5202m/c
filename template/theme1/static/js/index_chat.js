    ...
    setEvent: function() {
        ...
        $("#sendBtn").click(function() {
            ...
            if (indexJS.userInfo.isSetName === false) {
                ...
                $('#personal_center').trigger("click", [true]);
                $('#modifyNk').click();
...
/*            $.post(indexJS.apiUrl + "/message/sendMsg", {
        ...
        $(".signinbtn").click(function() {
            common.getJson('/addSignin', null, function(data) {
                if (data.isOK) {
                    box.showMsg("签到成功!");

                    $(".signinbox .progressbar").trigger("signin", data.signDays);
                } else {
    ...
    getCSList: function() {
        ...
        try {
            $.getJSON('/getCS', { groupId: groupId }, function(result) {
    ...
    //setCookie
    setCookies: function(id) {
        //s20是代表20秒
        //h是指小时，如12小时则是：h12
        //d是天数，30天则：d30
        var strsec = chat.setExpiryTime("h12");
        var exp = new Date();
        exp.setTime(exp.getTime() + strsec * 1);
        var index_size = document.cookie.split(id + "=");
        if (index_size.length == 2) {
            var i = index_size[1].split(";")[0]; //获取
            i++;
            document.cookie = id + "=" + i + ";expires=" + exp.toGMTString();
            return i;
        } else {
            //第一次赋值为：1
            document.cookie = id + "=" + 1 + ";expires=" + exp.toGMTString();
            return 1;
        }
    },
    //获取Cookie，获取cookie次数
    getCooikes: function(id) {
        //s20是代表20秒
        //h是指小时，如12小时则是：h12
        //d是天数，30天则：d30
        var strsec = chat.setExpiryTime("h12");
    ...
    setContent: function(data, isMeSend, isLoadData) {
        ...
        if (data.rule) {
            ...
            if (data.value && data.value.type == "userGag") {
                var userGagCooikes = chat.getCooikes("userGag");
                //判断弹出层只弹出一次
                if (userGagCooikes != null && userGagCooikes != "" && userGagCooikes > 0) {} else {
                    chat.setCookies("userGag");
                    common.openPopup('.blackbg,.silent_tip');
                }
            }
            return;
        }
    ...
    formatContentHtml: function(data, isMeSend, isLoadData) {
        ...
        if (content.msgType == chat.msgType.img) {
            if (content.needMax) {
                pHtml = '<a href="/getBigImg?publishTime=' + fromUser.publishTime + '&userId=' + fromUser.userId + '" data-lightbox="dialog-img"><img src="' + content.value + '" alt="图片"/></a>';
            } else {
    ...
    getAImgOrLevel: function(userId, clientGroup, userType, avatar) {
        var retObj = { aImg: common.isValid(avatar) ? '<img src="' + avatar + '">' : '<img src="/theme1/img/user.png">', level: '' };
    ...
    getOnlineUserDom: function(row) {
        ...
        $("#userListIdNew li[id='" + row.userId + "']").remove(); //存在则移除旧的记录
        var dialogHtml = chat.getDialogHtml(row.userId, row.nickname, row.userType),
            isMeHtml = "",
            csHtml = '',
            seq = row.sequence,
            meCls = '';
        ...
        if (indexJS.userInfo.userId == row.userId) {
            return {
                hasDg: common.isValid(dialogHtml),
                seq: seq,
                dom: '<li  id="' + row.userId + '" cg="' + (common.isBlank(row.clientGroup) ? '' : row.clientGroup) + '" t="' + seq + '" utype="' + row.userType + '" class="' + lav.level + '" >' + dialogHtml + '<div class="imgdiv">' + lav.aImg + '</div><span class="' + meCls + '">' + row.nickname + isMeHtml + '</span>' + csHtml + '</li>'
            };
        } else {
            return {
                hasDg: common.isValid(dialogHtml),
                seq: seq,
                dom: '<li onclick=\'chat.setDialog("' + row.userId + '","' + row.nickname + '",0,"' + row.userType + '","' + row.avatar + '");\' id="' + row.userId + '" cg="' + (common.isBlank(row.clientGroup) ? '' : row.clientGroup) + '" t="' + seq + '" utype="' + row.userType + '" class="' + lav.level + '" >' + dialogHtml + '<div class="imgdiv">' + lav.aImg + '</div><span class="' + meCls + '">' + row.nickname + isMeHtml + '</span>' + csHtml + '</li>'
            };
        }
        //   return {hasDg:common.isValid(dialogHtml),seq:seq,dom:'<li id="'+row.userId+'" cg="'+(common.isBlank(row.clientGroup)?'':row.clientGroup)+'" t="'+seq+'" utype="'+row.userType+'" class="'+lav.level+'" >'+dialogHtml+'<a href="javascript:" t="header" class="uname"><div class="imgdiv">'+lav.aImg+'</div><span class="'+meCls+'">'+row.nickname+isMeHtml+'</span>'+csHtml+'</a></li>'};
    },
    /**
     * 离开房间提示
     */
        ...
        if (flag == "forcedOut") { //强制下线分析师
            if (userIds) {
                for (var i in userIds) {
                    ...
                    $("#analystbar .contactbox[uid='" + userIds[i] + "']").remove();
                }
            }
            return;
        }
    ...
    setWhOnlineTip: function(toUserId, isOnline) {
        ...
        var txt = $('#wh_msg_' + toUserId + ' .titonline strong'),
    ...
    setWhContent: function(data, isMeSend, isLoadData, isOnlyFill) {
        ...
        if (content.msgType == chat.msgType.img) {
            if (content.needMax) {
                pHtml = '<a href="/getBigImg?publishTime=' + fromUser.publishTime + '&userId=' + fromUser.userId + '" data-lightbox="whimg-"' + fromUser.userId + '><img src="' + content.value + '" alt="图片"/></a>';
            } else {
    ...
    setSocket: function() {
        ...
        this.socket.on('connect', function() {
            ...
            var currTab = $("#roomInfoId"),
                ...
                teacherId = '',
                teacherName = '';
            if (common.isValid(indexJS.courseTick.course)) {
                courseId = indexJS.courseTick.course.courseId;
                courseName = indexJS.courseTick.course.courseName;
                teacherId = indexJS.courseTick.course.lecturerId;
            ...
            var postData = {
                ...
                userName: $('#myUserName').attr('un'),
                roomName: currTab.text(),
                email: $('#myEmail').attr('e'),
                ...
                case 'onlineNum':
                    {
                        ...
                        if (data.online) {
                            ...
                            //得到聊天相关属性
                            var lis = $("#userListIdNew li"),
                                onlineUserDom = chat.getOnlineUserDom(userInfoTmp),
                                liDom = onlineUserDom.dom;
                            if (onlineUserDom.seq == "0") {
                                ...
                                lis.each(function() {
                                    ...
                                    if (userInfoTmp.sequence < seqTmp) {
                                        $(this).before(liDom);
                                        isInsert = true;
                                        return false;
                                    } else if (userInfoTmp.sequence == seqTmp) {

                                        nickTmp = $(this).find("a span").text();
                                        if (userInfoTmp.nickname <= nickTmp) {
                                            $(this).before(liDom);
                                            isInsert = true;
                                            return false;
                                        }
                                ...
                                if (!isInsert) {
                                    $("#userListIdNew").append(liDom);
                                }
                            }
                            indexJS.setListScroll(".otheruser .scrollbox");
                        } else {
                            if (indexJS.userInfo.userId != userInfoTmp.userId) {
                                $("#userListIdNew li[id='" + userInfoTmp.userId + "']").remove();
                                //客服不下线，显示灰色
    ...
    talkBoxPush: {
        ...
        showMsg: function(info) {
            var html = [];
            html.push('<div class="dialog info_push" style="padding-right:10px;">');
     ...
     * @param [isOffline]
     */
    contactAnalystEvent: function(userInfo, isOffline) {
        var $item = $('#analystbar div[uid="' + userInfo.userId + '"]');
        if (isOffline || $item.size() > 0) {
            $item.remove();
            return;
        }
        $item = ['<div href="javascript:void(0);" class="contactbox" uid="' + userInfo.userId + '" onclick="_gaq.push([\'_trackEvent\', \'pmchat_studio\', \'right_lts_' + userInfo.userId + '\', \'content_right\', 1, true]);">'];
        $item.push('<div class="himg">');
        $item.push('<img src="' + userInfo.avatar + '" alt="">');
        $item.push('</div>');
        $item.push('<a href="javascript:void(0);" class="contactbtn">');
        $item.push('联系' + userInfo.nickname);
        $item.push('</a></div>');
        $item = $($item.join(""));
        $('#analystbar').append($item);
        $item.find("a").bind("click", userInfo, function(e) {
            var user = e.data;
            if (user) {
                chat.setDialog(user.userId, user.nickname, 0, user.userType, user.avatar); //设置对话
            }
        });
    },
    /**
