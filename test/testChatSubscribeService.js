...
describe("chatSubscribeService.getSubscribeList", () => {
  ...
  it("chatSubscribeService.getSubscribeList should work as expect.", (done) => {
    chatSubscribeService.getSubscribeList(
        {groupType: "studio", userId: "13800138096"}, (data) => {
          should(data).not.be.null();
          data.length.should.equal(0);
        });
    chatSubscribeService.getSubscribeList(
        {groupType: "studio", userId: "13800138096"}).then((data) => {
      should(data).not.be.null();
      data.length.should.equal(0);
      done();
    }).catch((e) => {
...
describe("chatSubscribeService.saveSubscribe4UTM", () => {
  ...
  it("chatSubscribeService.saveSubscribe4UTM should work as expect.",
      (done) => {
        chatSubscribeService.saveSubscribe4UTM("studio", "18813982018",
            "daily_quotation", true).then((data) => {
          should(data).not.be.null();
