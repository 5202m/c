...
let logger = require('../resources/logConf').getLogger("testClientTrainService"); 
...
describe("clientTrainService.saveTrain", () => {
	...
	clientTrainService.saveTrain("studio_teach", "pxunfnnupxxns", "还是我啊", (data) => {
	    should(data).not.be.null();
	    data.should.be.String();
	}).catch((e) => {
	    logger.error(e);
	});
	clientTrainService.saveTrain("studio_teach", "pxunfnnupxxns","还是我啊").then((data) => {
	    should(data).not.be.null();
	    data.should.be.String();
	    done();
	}).catch((e) => {
...
describe("clientTrainService.addClientTrain", () => {
	...
	clientTrainService.addClientTrain(testingParam, testingUserInfo).then((data) => {
	    should(data).not.be.null();
	    data.should.have.property("errcode");
	    done();
	}).catch((e) => {
...
describe("clientTrainService.getTrainAndClientNum", () => {
	...
	clientTrainService.getTrainAndClientNum("hxstudio", "hxteacher").then((data) => {
	    should(data).not.be.null();
	    data.should.be.an.Object();
	    data.should.have.property("trainNum");
	    data.should.have.property("clientNum");
...
describe("clientTrainService.getTrainList", () => {
	...
	clientTrainService.getTrainList("hxstudio", "hxteacher",  true).then((data) => {
	    should(data).not.be.null();
	    data.should.be.an.Array();
	    data.length.should.aboveOrEqual(1);
	    done();
