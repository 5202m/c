...
describe("userService.getUserInfo", () => {
	...
	userService.getUserInfo(
		...
		(data) => {
		    ...
		    data.should.have.keys("_id","userNo","userName","position","introduction","introductionImg","avatar");
...
describe("userService.verifyRule", () => {
	...
	userService.verifyRule(clientGroup, nickname, isWh, userType, groupId, content)
	.then((data) => {
	    should(data).not.be.null();
	    done();
	}).catch((e) => {
...
describe("userService.getMemberList", () => {
	...
	userService.getMemberList(
		...
		(data) => {
		    should(data).not.be.null();
		    data.should.be.an.Object();
		    data.should.have.keys("_id","mobilePhone","createIp","updateUser","updateIp","loginPlatform");
		    (data["_id"]).should.be.equal(id);
		}
	).catch((e) => {
	    logger.error(e);
...
describe("userService.updateChatUserGroupStatus", () => {
	...
	chatStatus = 0,
	userService.updateChatUserGroupStatus(userInfo, chatStatus, sendMsgCount)
	.then((data) => {
	    should(data).be.null();
	    done();
	}).catch((e) => {
    ...
    it("Should work as expect.", (done) => {
    	userService.getTeacherByUserId(
    		...
    		(data) => {
    		    should(data).not.be.null();
    		    data.should.be.an.Object();
    		    data.should.have.keys("_id", "userNo", "userName", "introductionImg", "wechatCodeImg", "introductionImgLink");
    		}
    	).catch((e) => {
    	    logger.error(e);
    	});
