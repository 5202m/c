...
var common = {
    ...
    renderPath: function(req, tempPlatform, view, themeKey) {
        ...
        return global.rootdir + '/template/' + themeKey + '/view/' + (view ? view :
            'index');
    },
    /**
     * 提取模板平台对应key
     ...
     */
    isBlank: function(v) {
        return v == undefined || v == null || v == 'null' || v == 'undefined' ||
            this.trim(v) == '';
    },
    /**
     * 日期比较大小
    ...
    encodeHtml: function(str) {
        return this.isBlank(str) ? '' : (str.replace(/&lt;/g, "<").replace(/&gt;/g,
            ">").replace(/&amp;/g, "&"));
    },
    /**
     * 提取ip
    ...
    getClientIp: function(req) {
        ...
        return this.isBlank(ip) ? '' : ip.replace(
            /.*:(\d{1,4}.\d{1,4}.\d{1,4}.\d{1,4})$/g, "$1");
    },
    /**
     * 通过房间id提取房间组别类型
     * @param roomId
    ...
    dateTimeWeekCheck: function(srcDateTime, nullResult, checkDate) {
        ...
        isPass = this.isBlank(dateTime.beginDate) || currDateStr >=
            dateTime.beginDate;
        if (isPass) {
            isPass = this.isBlank(dateTime.endDate) || currDateStr <=
                dateTime.endDate;
        }
        if (!isPass) {
            return false;
    ...
    getHHMMSS: function(date) {
        ...
        var datetime = (date.getHours() < 10 ? "0" + date.getHours() :
                date.getHours()) +
            ":" +
    ...
    copyObject: function(srcObj, targetObj, hasTargetField) {
        ...
        if (hasTargetField) {
            var srcTmp = (typeof srcObj.toObject === "function") ? srcObj.toObject() :
                srcObj;
    ...
    setCrossDomain: function(req, res) {
        ...
        if (req.headers["user-agent"] &&
            req.headers["user-agent"].toLowerCase().indexOf("msie") > 0) {
    ...
    formatDate: function(date, format) {
        ...
        var loc_patterns = ['yyyy', 'yy', 'MM', 'M', 'dd', 'd', 'HH', 'H', 'hh',
            'h', 'mm', 'm', 'ss', 's', 'SSS', 'S', 'q'
        ];
        for (var i = 0, lenI = loc_patterns.length; i < lenI; i++) {
            if (new RegExp(loc_patterns[i]).test(loc_result)) {
                loc_result = loc_result.replace(new RegExp(loc_patterns[i], "gm"),
                    getPart(date, loc_patterns[i]));
            }
        }
        return loc_result;
    },
     ...
     * @returns {*}
     */
    formatHostUrl: function(host, url) {
        if (this.isBlank(host) || host.indexOf("hx9999") < 0) {
            return url;
        }
        try {
            var toUrl = function(hostTmp, urlTmp) {
                return urlTmp.replace(/(.*.)(com|hk|org)(:[\d]+)?/g,
                    '$1' + hostTmp.replace(/(.*.)(com|hk|org)/g, '$2') + '$3');
            };
            if (typeof url === 'object') {
                var newUrl = {};
                for (var i in url) {
                    newUrl[i] = toUrl(host, url[i]);
                }
                return newUrl;
            } else {
                return toUrl(host, url);
            }
        } catch (e) {
            console.error("formatHostUrl has error!", e);
            return url;
        }
    },
    /**
