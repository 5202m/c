...
class Index {
    ...
    _setStaticPath() {
        ...
        this._getThemePaths().forEach(themePath => {
            _this._app.use(`/${themePath}`, _this._express.static(
                path.join(__dirname, `../template/${themePath}/static`)));
        });
        _this._app.use('/base',
            _this._express.static(path.join(__dirname, `../template/base`)));
        _this._app.use('/admin', _this._express.static(
            path.join(__dirname, `../template/admin/static`)));
    }
