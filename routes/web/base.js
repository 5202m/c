...
router.get('/', function(req, res) {
    ...
    if(!isKeepOptions && fromPlatform && !chatUser.toGroup && !chatUser.groupId && common.containSplitStr(config.studioThirdUsed.platfrom,fromPlatform)){
...
function toStudioView(chatUser, options, groupId,clientGroup,isMobile,req,res){
    studioService.getIndexLoadData(chatUser,groupId, true,(!isMobile||(isMobile && common.isValid(groupId))), chatUser.isLogin, function(data){
        ...
        if(!data.studioList){
            if(data.syllabusResult){
                ...
                viewDataObj.syllabusData=JSON.stringify({
                    ...
                    studioLink: (common.isBlank(syResult.studioLink) ? "" : JSON.parse(syResult.studioLink)),
                    courses: (common.isBlank(syResult.courses) ? "" : syllabusService.removeContext(JSON.parse(syResult.courses)))
                });
            }
        }else{
        ...
        if(snUser.firstLogin && snUser.groupId){//刷新页面不记录访客记录
            ...
            var courseName = req.body['title']||''+'_'+req.body['lecturer']||''+'_'+req.body['courseTypeName']||'';
            var vrRow={cookieId:snUser.cookieId,sessionId:req.sessionID, userId:snUser.userId,platform:fromPlatform || "",userAgent:req.headers["user-agent"],groupType:getGroupType(req),roomName:'',roomId:snUser.groupId,nickname:snUser.nickname,userName:snUser.userName,email:snUser.email,clientGroup:chatUser.clientGroup,clientStoreId:snUser.clientStoreId,mobile:snUser.mobilePhone,ip:ip,email:snUser.email,visitorId:snUser.visitorId,nickName:snUser.nickname,courseName:courseName,courseId:req.body['courseId'],teacherId:req.body['lecturerId'],teacherName:req.body['lecturer'],accountNo:snUser.accountNo};
    ...
    if(result.error){
        ...
        if(loginType == "pwd"){
            ...
            studioService.login({mobilePhone:mobilePhone, password:password, groupType:userSession.groupType}, 4, function(loginRes){
                if(loginRes.isOK && constant.clientGroup.real != loginRes.userInfo.clientGroup){
                    ...
                    var snUser = req.session.studioUserInfo;
                    var dasData = {mobile:mobilePhone,cookieId:cookieId,clientGroup:snUser.clientGroup,roomName:roomName,roomId:snUser.groupId,platform:'',userAgent:req.headers['user-agent'],sessionId:req.sessionID,roomId:snUser.groupId,clientStoreId:snUser.clientStoreId,groupType:snUser.groupType,userName:snUser.userName,email:snUser.email,ip:common.getClientIp(req),visitorId:visitorId,nickName:snUser.nickname,courseName:courseName,courseId:courseId,teacherId:teacherId,teacherName:teacherName,accountNo:snUser.accountNo};
                            ...
                            studioService.checkClientGroup(mobilePhone,null,common.getTempPlatformKey(userSession.groupType),function(clientGroup, accountNo){
                                if(loginRes.isOK){
                                    ...
                                    studioService.updateClientGroup(userSession.groupType,mobilePhone, clientGroup, accountNo, function (isOk) {
                                        ...
                                        //req.session.studioUserInfo.courseId = courseId;
                                        //req.session.studioUserInfo.courseName = courseName;
                                        res.json({isOK:true, userInfo : req.session.studioUserInfo}); //即使修改账户级别失败也登录成功
                                    });
                                }else{
                                    ...
                                    studioService.studioRegister(userInfo,clientGroup,function(result){
                                        if(result.isOK){
                                            req.session.studioUserInfo={cookieId:cookieId,visitorId : visitorId,roomName:roomName,groupType:userSession.groupType,clientStoreId:clientStoreId,firstLogin:true,isLogin:true,mobilePhone:userInfo.mobilePhone,userId:userInfo.userId,defGroupId:userInfo.defGroupId,clientGroup:userInfo.clientGroup,nickname:userInfo.nickname};
        ...
        studioService.login({userId:userId,groupType:userSession.groupType}, 2, function(loginRes){
            if(loginRes.isOK){
                ...
                req.session.studioUserInfo.visitorId = visitorId;
                var snUser = req.session.studioUserInfo;
                var dasData = {mobile:snUser.mobilePhone,cookieId:cookieId,clientGroup:snUser.clientGroup,platform:'',userAgent:req.headers['user-agent'],sessionId:req.sessionID,roomId:snUser.groupId,clientStoreId:snUser.clientStoreId,groupType:snUser.groupType,userName:snUser.userName,email:snUser.email,ip:common.getClientIp(req),visitorId:visitorId,nickName:snUser.nickname,courseName:courseName,courseId:courseId,teacherId:teacherId,teacherName:teacherName,accountNo:snUser.accountNo};
                visitorService.saveVisitorRecord("login", dasData);
                if(loginRes.userInfo.clientGroup!=constant.clientGroup.vip && loginRes.userInfo.clientGroup!=constant.clientGroup.active) {//检查账号接口同步数据
                    studioService.checkClientGroup(loginRes.userInfo.mobilePhone, null, common.getTempPlatformKey(userSession.groupType), function (clientGroup, accountNo) {
                        if(constant.clientGroupSeq[clientGroup]<constant.clientGroupSeq[loginRes.userInfo.clientGroup]){
...
router.get('/logout', function(req, res) {
    ...
    var roomName = req.query['roomName'],
        ...
        teacherId=req.body['teacherId'],
        teacherName=req.body['teacherName'];
    var dasData = {mobile:snUser.mobilePhone,cookieId:cookieId,clientGroup:snUser.clientGroup,roomName:snUser.roomName||roomName,platform:platform,userAgent:req.headers['user-agent'],sessionId:req.sessionID,roomId:snUser.groupId,clientStoreId:snUser.clientStoreId,groupType:snUser.groupType,userName:snUser.userName,email:snUser.email,ip:common.getClientIp(req),visitorId:snUser.visitorId,nickName:snUser.nickname,courseName:courseName,courseId:courseId,teacherId:teacherId,teacherName:teacherName,accountNo:snUser.accountNo};
    visitorService.saveVisitorRecord("logout", dasData);
    req.session.studioUserInfo=null;
    //注销之后检查当前房间是否给游客授权，若授权则以游客身份再次进入当前房间
    studioService.checkGroupAuth(null, snUser.groupId,constant.clientGroup.visitor,null,function(groupInfo){
        if(groupInfo){
...
router.post('/modifyName',function(req, res){
    ...
    if(!userInfo || common.isBlank(userInfo.mobilePhone)){
        ...
        userService.modifyNickname(userInfo.mobilePhone,getGroupType(req),nickname)
        .then(result => {
            if(result.isOK){
        ...
        baseApiService.checkMobileVerifyCode(params.mobilePhone, userSession.groupType+"_reg", params.verifyCode, function(chkCodeRes){
            if(!chkCodeRes || chkCodeRes.result != 0 || !chkCodeRes.data){
                ...
                studioService.checkClientGroup(params.mobilePhone,null,common.getTempPlatformKey(userSession.groupType),function(clientGroup, accountNo){
                    ...
                    studioService.studioRegister(userInfo,clientGroup,function(result){
                        if(result.isOK){
                            var userSessionInfo = {cookieId:params.cookieId,groupType:userSession.groupType,clientStoreId:params.clientStoreId,firstLogin:true,isLogin:true,mobilePhone:userInfo.mobilePhone,userId:userInfo.userId,defGroupId:userInfo.defGroupId,clientGroup:userInfo.clientGroup,nickname:userInfo.nickname,ip:userInfo.ip,visitorId:params.visitorId};
                            req.session.studioUserInfo=userSessionInfo;
                            result.userId = userInfo.userId;
                            delete userInfo["pwd"];
                            var dasData = {mobile:userSessionInfo.mobilePhone,userName:'',cookieId:params.cookieId,clientStoreId:params.clientStoreId,clientGroup:userInfo.clientGroup,ip:userInfo.ip,nickname:(userSessionInfo.nickname||params.nickname),platform:params.platform||'',userAgent:req.headers["user-agent"],groupType:userInfo.groupType,visitorId:params.visitorId,roomId:userInfo.groupId,operationType:3,sessionId:req.sessionID,courseId:params.courseId,courseName:params.courseName,teacherId:params.teacherId,teacherName:params.teacherName,accountNo:userInfo.accountNo};
        ...
        syllabusService.getSyllabus(userInfo.groupType,roomIds,function(data){
            ...
            if(data){
                ...
                for(var i in newData){
                    ...
                    if(course){
                        backObj={title:course.title,day:course.day,name:course.lecturer,startTime:course.startTime,endTime:course.endTime,isNext:course.isNext};
                    }else{
                        backObj={title:'',day:'',name:'',startTime:'',endTime:'',isNext:false};
                    }
            ...
            async.each(roomIds.split(","), function (rid, callbackTmp) {
                if(!result.data.hasOwnProperty(rid)){
                    result.data[rid] = {title:'',day:'',name:'',startTime:'',endTime:'',isNext:false};
                }
...
router.post('/getShowTeacher', function(req, res){
    ...
    if(typeof params == 'string'){
        ...
        if(!chatUser){
            logger.warn("[getShowTeacher] Illegal Session studioUserInfo, ", req.session.studioUserInfo);
            res.json(nullResult);
            return;
        }
        params.groupType = chatUser.groupType;
        params.groupId = common.isBlank(params.groupId)?chatUser.groupId:params.groupId;
...
router.post('/setThemeStyle', function(req, res){
    ...
    if(common.isBlank(params.defTemplate)){
        ...
        studioService.setUserGroupThemeStyle(req.session.studioUserInfo, params, function(result){
            if(result){
...
router.post('/getRoomList', function(req, res){
    let chatUser = req.session.studioUserInfo, isMobile = common.isMobile(req),
        clientGroup=chatUser && chatUser.isLogin?chatUser.clientGroup:constant.clientGroup.visitor,
        isVisitor=(constant.clientGroup.visitor==clientGroup);
    let viewDataObj = {}, newStudioList=[], rowTmp=null;
    studioService.getIndexLoadData(chatUser, null, true,(!isMobile||(isMobile && common.isValid(null))), chatUser.isLogin, function(data) {
        if (!data.studioList) {
            if (data.syllabusResult) {
                var syResult = data.syllabusResult;
                viewDataObj.syllabusData = JSON.stringify({
    ...
    if(common.isValid(systemCategory) && common.isValid(platform)) {
        async.parallel({
            analysts : function(callback) {
                userService.getAnalystList({systemCategory: systemCategory}, function (result) {
