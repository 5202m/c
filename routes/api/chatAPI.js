...
router.post('/approvalMsg', function (req, res) {
  ...
  var result = {isOK: false, error: null};
  if (common.isBlank(publishTimeArr) || common.isBlank(fUserIdArr)
      || common.isBlank(status) || common.isBlank(groupId)) {
    result.error = errorMessage.code_1000;
  } else {
    var data = {
      fromUser: {
        groupId: groupId,
        userId: approvalUserNo,
        fromPlatform: constant.fromPlatform.pm_mis
      },
      status: status,
      publishTimeArr: publishTimeArr.split(","),
      fuIdArr: fUserIdArr.split(",")
    };
...
router.post('/showTradeNotice', function (req, res) {
  ...
  if (common.isBlank(tradeInfoJSON)) {
    ...
    //通过手机号码查询客户组
    userService.getClientGroupByMId(mobileArr, tradeInfo.groupType,
        function (mbObj) {
          for (var i = 0; i < tradeInfoArray.length; i++) {
            tradeInfo = tradeInfoArray[i];
            if (tradeInfo.tradeType == 2) { //客户晒单
